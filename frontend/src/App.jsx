...
export default function App() {
  ...
  const [live, setLive] = useState(null);           // latest single metrics object
  const [history, setHistory] = useState([]);        // chart data
  const [recentTrades, setRecentTrades] = useState([]); // table rows
  ...
  // ── Fetch history & recent trades via REST (initial load + periodic) ─────
  const fetchHistory = useCallback(() => {
    ...
    fetch(`${API}/metrics/history?symbol=${symbol}&start=${start}&end=${end}`)
      .then(r => r.json())
      .then(data => {
  ...
  useEffect(() => {
    fetchHistory();
    fetchRecent();
    const t = setInterval(() => { fetchHistory(); fetchRecent(); }, 10000);
    return () => clearInterval(t);
  }, [fetchHistory, fetchRecent]);

  // ── WebSocket for live metric cards ──────────────────────────────────────
  useEffect(() => {
    ...
    ws.onmessage = (e) => {
      ...
      setLive(data);
      // Append to history chart in real time
      setHistory(prev => {
        const point = {
          ...data,
      ...
      // Prepend to recent trades table
      setRecentTrades(prev => [data, ...prev].slice(0, 15));
    };
    ...
    <div style={{
            ...
            {availableSymbols.map(s => (
              <button key={s} onClick={() => { setSymbol(s); setInputVal(s); }}
                style={{
                  ...
                  background: symbol === s ? C.blue + "22" : "transparent",
                  color: symbol === s ? C.blue : C.muted,
                  border: `1px solid ${symbol === s ? C.blue : C.border}`,
                }}>
                {s}
              </button>
            ))}
      ...
      <div style={{ display: "flex", gap: 14, marginBottom: 24, flexWrap: "wrap" }}>
        <MetricCard
          ...
          accent={C.blue}
          large
          value={live ? `$${fmtLarge(live.price)}` : "—"}
          sub={<PctBadge value={live?.change_24h_pct} />}
        />
        <MetricCard
          label="Rolling Avg (20)"
          accent={C.purple}
          value={live ? `$${fmtLarge(live.rolling_avg_20)}` : "—"}
          sub={<span style={{ color: C.muted, fontSize: 12 }}>20-trade window</span>}
        />
        <MetricCard
          label="RSI (14)"
          accent={live?.rsi > 70 ? C.red : live?.rsi < 30 ? C.green : C.blue}
          value={<RSIBar value={live?.rsi} />}
        />
        <MetricCard
          label="Volatility (σ)"
          accent={C.amber}
          value={live ? fmt(live.volatility, 2) : "—"}
          sub={<span style={{ color: C.muted, fontSize: 12 }}>Std dev (20 trades)</span>}
        />
        <MetricCard
          label="Volume"
          accent={C.green}
          value={live ? fmtLarge(live.volume) : "—"}
          sub={<span style={{ color: C.muted, fontSize: 12 }}>Cumulative session</span>}
        />
        <MetricCard
          label="Trade Count"
          accent={C.purple}
          value={live ? live.trade_count?.toLocaleString() : "—"}
          sub={<span style={{ color: C.muted, fontSize: 12 }}>This session</span>}
        />
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: C.text }}>
            Price History <span style={{ color: C.muted, fontWeight: 400 }}>— last hour</span>
          </h2>
            ...
            <thead>
              <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                {["Symbol","Price","Avg₂₀","Return","Volatility","RSI","Volume","24h Δ","Time"].map(h => (
                  <th key={h} style={{ padding: "10px 16px", color: C.muted, fontWeight: 600,
            ...
            <tbody>
              {recentTrades.map((m, i) => (
                <tr key={i} style={{
                  ...
                  <td style={{ padding: "10px 16px", fontFamily: "monospace", color: m.rsi > 70 ? C.red : m.rsi < 30 ? C.green : C.blue }}>
                    {m.rsi != null ? fmt(m.rsi) : "—"}
                  </td>
                  ...
                  <td style={{ padding: "10px 16px", color: C.muted, fontSize: 12 }}>
                    {m.trade_time ? new Date(m.trade_time).toLocaleTimeString() : "—"}
                  </td>
                </tr>
              ))}
              {recentTrades.length === 0 && (
                <tr><td colSpan={9} style={{ padding: "24px", textAlign: "center", color: C.muted }}>
                  Waiting for data…
                </td></tr>
              )}
            </tbody>
