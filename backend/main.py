            ...
            if symbol:
                cursor.execute("""
                    SELECT symbol, price, rolling_avg_20, return, volatility,
                           rsi, volume, trade_count, change_24h_pct, sharpe, max_drawdown, signal, signal_strength, trade_time
...
def _row_to_dict(row):
    return {
        ...
        "signal": row[11], "signal_strength": row[12],
        "trade_time": row[13].isoformat(),
    }